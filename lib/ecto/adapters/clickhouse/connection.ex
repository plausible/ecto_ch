defmodule Ecto.Adapters.ClickHouse.Connection do
  ...
  defp expr({:__typed, type, {:^, [], [_ix]}}, _sources, _query) do
    counter = Process.get(@counter)
    Process.put(@counter, counter + 1)
    ["{$", to_string(counter), ?:, tagged_to_db(type), ?}]
  end

  defp expr({:^, [], [_ix]}, _sources, query) do
    error!(
      query,
      "can't infer param types for this query, please use `Ecto.Query.API.type/2`"
    )
  end
  ...
  defp expr({:in, _, [left, {:^, _, [_, _length]}]}, sources, query) do
    counter = Process.get(@counter)
    Process.put(@counter, counter + 1)
    type = extract_type(left, sources)
    ...
    case handle_call(fun, length(args)) do
      {:binary_op, op} ->
        ...
        [
          ...
          op | op_to_binary({:__typed, type, right}, sources, query)
        ]
  ...
  defp expr({:__typed, _type, %Tagged{value: {:^, [], [_idx]}} = expr}, sources, query) do
    expr(expr, sources, query)
  end

  defp expr(%Tagged{value: {:^, [], [_idx]}, type: type}, _sources, _query) do
    counter = Process.get(@counter)
    Process.put(@counter, counter + 1)
    ["{$", to_string(counter), ?:, tagged_to_db(type), ?}]
  end
  ...
  # TODO
  defp extract_type({{:., _, [{:&, _, [idx]}, field]}, _, _}, sources) do
    {_, _, schema} = elem(sources, idx)
  ...
  defp op_to_binary({:__typed, _type, {op, _, [_, _]}} = expr, sources, query)
